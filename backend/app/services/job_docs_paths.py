"""Filesystem paths for job PDFs under {docs_root}/Docs/<folder_name>/."""
...
def docs_dir_relative(folder_name: str) -> str:
    return f"{_DOCS_SEGMENT}/{folder_name}"


def absolute_job_docs_dir(docs_root: Path, folder_name: str) -> Path:
    return (Path(docs_root) / _DOCS_SEGMENT / folder_name).resolve()


def stored_path_for_file(folder_name: str, filename: str) -> str:
    """POSIX-style relative path under docs_root."""
    return f"{_DOCS_SEGMENT}/{folder_name}/{filename}"


def exclusive_docs_folder_name(db: Session, job_id: int, customer_name: str) -> str:
    ...
    if not base:
        base = f"job_{job_id}"
    stmt = select(Job.id).where(Job.docs_folder_name == base, Job.id != job_id)
